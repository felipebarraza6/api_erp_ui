...
import {
  ...
  getPersons,
  updatePerson,
  visibleModalUpdateForm,
} from "../../actions/employess";
  ...
  const expandedRowRender = (person) => {
    return (
      <Descriptions layout="vertical">
        <Descriptions.Item span={3} label={<b>Empresa</b>}>
          {person.enterprise ? person.enterprise : "S/D"}
        </Descriptions.Item>
        <Descriptions.Item span={3} label={<b>Cargo</b>}>
          {person.charge ? person.charge : "S/D"}
        </Descriptions.Item>
        <Descriptions.Item span={3} label={<b>Telefono</b>}>
          {person.phone_number ? person.phone_number : "S/D"}
        </Descriptions.Item>
        <Descriptions.Item span={3} label={<b>Email</b>}>
    ...
    <Table
      ...
      title={() => (
        <React.Fragment>
          {attr.enterprise ? (
            <Text mark>
              {attr.enterprise.name} ({attr.quantity})
            </Text>
          ) : (
            <Text mark>TODOS({attr.quantity})</Text>
          )}
          <Button
            type="primary"
            size={"small"}
