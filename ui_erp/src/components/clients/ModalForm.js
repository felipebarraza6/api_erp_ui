...
const ModalForm = ({ visible, onCreate, onCancel, enterprise, error }) => {
  ...
  const onSearch = async (value) => {
    const rq = await api.worklands.list(value).then((r) => {
      setListPersons(r.data.results);
    });
  };
  ...
  return (
    <React.Fragment>
      <Modal
        visible={visible}
        title={
          enterprise ? `Nueva persona en ${enterprise.name}` : `Editar persona`
        }
        okText="Guardar"
        cancelText="Cancelar"
        onCancel={onCancel}
      ...
      >
        ...
        <Form form={form} layout="vertical" name="FormPerson">
          <Form.Item
            name="name"
            label="Nombre"
            rules={[
          ...
          >
            {other.other_charge ? (
              <>
                ...
                <Button
                  onClick={async () => {
                    const rq = api.worklands
                      .create({ name: newCharget })
                      .then((r) => {
                  ...
                  <>
                    <Option value={p.name}>
                      ...
                      <Button
                        ...
                        size="small"
                        onClick={async () => {
                          const rq = await api.worklands
                            .delete(p.id)
                            .then((r) => {
                              notification.success({
                                message: "CARGO ELIMINAD",
                              });
          ...
          <Form.Item
            name="phone_number"
            label="Telefono"
            rules={[{ required: false, message: "Ingresa el telefono" }]}
          >
