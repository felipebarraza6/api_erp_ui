...
const HeaderMobile = () => {
    ...
    return(<Row justify='center'>
        ...
        <Col span={24} style={{backgroundColor:'#001529', borderRadius:'10px 10px 0px 0px', paddingTop:'10px', paddingBottom:'10px', marginBottom:'0px'}}>
            <center><img src={imgsmart} style={{width:'30%'}} /></center>
        </Col>
        <Col span={24}>                    
            <Menu 
                selectedKeys={state.steps.current} defaultOpenKeys={['P2']} 
                onClick={changeMenu} 
                theme='dark' 
                mode='horizontal'
                style={styles.menu}>                
                {is_external &&
                    <Menu.Item key={state.client_external ? 'C1':'CE'}>{!state.client_external.is_draft ? 
                        <><UserOutlined/> {state.steps.current==='C1'&&'Cliente'}</>:<><EditFilled /> {state.steps.current==='CE'&&'Editar'}</>}</Menu.Item>} 
                <Menu.Item key='22' disabled={state.is_external && !state.client_external.is_draft ? true:false}><PlusCircleFilled/>{state.steps.current==='22'&&'Pozo'}</Menu.Item>               
                <Menu.SubMenu key='P2' disabled={state.is_external && !state.client_external.is_draft ? true:false} title={<><OrderedListOutlined/>({state.wells.length})</>}> 
