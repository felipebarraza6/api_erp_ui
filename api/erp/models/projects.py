...
class Project(ModelApi):
    ...
    lifting = models.ForeignKey(
        Lifting, blank=True, null=True, on_delete=models.CASCADE)

    def __str__(self):
        return str(self.id)


class UserCatchmentPoints(ModelApi):
    project = models.ForeignKey(
        Project, on_delete=models.CASCADE, null=True, blank=True)
    id_user_api = models.CharField(max_length=200, blank=True, null=True)
...
class NoteProject(ModelApi):
    ...
    file = models.FileField(
        upload_to='projects/files_notes', blank=True, null=True)
    user = models.ForeignKey(User, on_delete=models.CASCADE)
    for_user = models.ForeignKey(User, on_delete=models.CASCADE,
                                 related_name='for_user_note_project', blank=True, null=True)
...
class AnswerNote(ModelApi):
    note = models.ForeignKey(
        NoteProject, on_delete=models.CASCADE, null=True, blank=True)
    answer = models.TextField(max_length=2200, blank=True, null=True)
    user = models.ForeignKey(User, on_delete=models.CASCADE)
    file = models.FileField(
        upload_to='projects/files_notes/answers', blank=True, null=True)
...
class TypeElement(ModelApi):
    deparment = models.ForeignKey(
        CompanyDeparment, on_delete=models.CASCADE, blank=True, null=True)
